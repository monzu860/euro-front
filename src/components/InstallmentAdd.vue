<template>
	...
	<div class="container">
		<div class="row justify-content-center">
			<div class="col-md-8">
				<div class="card">
					<div class="card-header">Installment List</div>
					<div class="card-body">
							...
							<table class="table">
								...
								<tbody>
									<tr v-for="(installment, index) in installments" :key="installment.id">
										...
										<td><input type="text" v-model="installment.note"></td>
										<td><button class="btn btn-danger" @click="remove(index)">-</button></td>
									</tr>
									<button type="button" class="btn btn-secondary" @click="addNew({amount:null,expire_date:null,note:null})">Add New</button>
								</tbody>
							</table>
...
export default {
	...
	methods:{
		...
		async save() {
			let self=this;
			await axios.post(`${window.remote_url}/installment`,{installments:this.installments,customer_id:this.customer_id}).then(response=>{
